...
describe('Channels page UU 短信登录', () => {
  ...
  it('图形校验后重发必须复用首次响应的 session_id', async () => {
    ...
    solveCaptchaMock.mockResolvedValue({
      ret: 0,
      ticket: 'ck-ticket',
      randstr: 'ck-rand',
    })
    ...
    fireEvent.click(screen.getByText('悠悠有品 · UU'))
    fireEvent.click(screen.getByText('短信登录（当前被平台限制）'))

    fireEvent.change(screen.getByPlaceholderText('+86 手机号'), {
    ...
    expect(postMock).toHaveBeenNthCalledWith(1, '/channels/uu/sms', {
      phone: '13800000000',
    })
  ...
  it('首次发送不带 session_id，由后端下发并在后续复用', async () => {
    ...
    expect(postMock).toHaveBeenCalledWith('/channels/uu/sms', {
      phone: '13800000001',
    })
    expect(
      screen.getByRole('button', { name: '发送验证码(60s)' }),
    ).toBeDisabled()
  })
  ...
  it('手动导入 UU Token：PUT 原样 token 并清空输入', async () => {
    ...
    fireEvent.change(screen.getByLabelText('UU Token'), {
    ...
    expect(putMock).toHaveBeenCalledWith('/channels/uu', {
      token: 'jwt-token-pasted',
    })
    expect(
      (screen.getByLabelText('UU Token') as HTMLTextAreaElement).value,
    ).toBe('')
  })
})
