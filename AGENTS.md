...
**2026-09-25 代码质量审查 CQ-01～03 修复完成**：货架快照按数据库观测边界原子应用，
保护随后发生的上下架/改价并拒绝倒序响应；订单反馈持久化唯一挂单归属，歧义保留待补全，
因子提交检查并发订单校订；旧401不能清除新登录令牌。迁移0011、ADR-0013，新增11个
后端集成回归及4个前端回归；完整 make gate（含迁移升降级、前端55测试）通过。
存量已折算因子不猜测回算，未执行生产发布或真实平台写入；详见
[修复证据](docs/knowledge/evidence/2026-09-25-code-quality-remediation.md)。

**2026-09-19 前端 UX 修复完成**：UX-01～15 的本轮改进已落地，UX-16 提供基本窄屏巡检。
数字草稿/保存防重与保护、全局任务范围确认、有效执行许可、完整脱敏定价历史、列表完整集筛选排序、
财务口径与观察元数据、渠道独立配置及敏感输入清理均已接线。OpenAPI v0.10.0，新增 ADR-0012，
无数据库迁移；完整 make gate（51 前端测试、后端 race/隔离集成、逐包覆盖与迁移升降级）通过，
提交 2d50192 上的 Chromium 93 检查及八页×三视口通过。未进行本轮生产发布或真实平台写入；
单件/按筛选重定价、逐次任务历史、历史策略版本/行情快照补录仍是独立后续能力。
详见 [修复证据](docs/knowledge/evidence/2026-09-19-frontend-ux-remediation.md)。
