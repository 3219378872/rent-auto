...
- 订单反馈归属（CQ-02）：`lease_orders.factor_listing_id` 持久化唯一挂单身份。
  仅按同渠道、非空资产及兼容模板寻找候选，排除订单开始之后才上架、开始之前已确认
  退架的行；缺少开始时间时以首次本地观测排除后来挂单，但不据此推断出租时刻。
  候选必须唯一，不可按最新挂单或状态猜测。无候选/歧义保留未处理并告警，资料补全后重试，
  不占用已绑定订单的处理批次。历史已折算订单不重放，不推测修正既有 factor。
...
护栏顺序：绝对界与比例底线 → 变化帽 → 最终三元组押金校验 → 冷却 → 全字段噪声判定。
比例底线、绝对界与单次变化区间无共同可行报价时返回 `guardrail_conflict`，不得突破
变化帽；`max_change_ratio=0` 表示短租金不得变化。金额量化不得使报价越过变化区间。
ECO 押金必须由最终提交的 rent/long/D 重新派生；UU 押金、长租金或最大租期变化
不得因短租金不变而被丢弃。冷却仍对整个改价载荷生效。
执行总闸：环境 dry-run、全局策略未启用真实执行、模板策略未启用真实执行任一成立
均不得调用平台；模板不能覆盖已关闭的全局总闸。
