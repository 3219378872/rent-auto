...
- 每请求一次 app_settings 主键读——单用户面板规模下成本可忽略，
  不引入缓存失效复杂度。
- 2026-09-08 修订：仅键不存在表示初始 0；存储不可用、null、负值、非法 JSON 类型均拒绝授权。
  数据库原子递增防并发覆盖；登录先读纪元再取密码快照，避免改密并发时旧密码获得新纪元。
- `PUT /api/v1/auth/password` 校验当前密码后，将 bcrypt CAS 更新与纪元递增放在同一事务。
  环境变量管理的密码不可通过此接口修改；首次随机口令只交付到 0600 独占文件，不进入日志。
