...
15. **发货轮询 lastErr（review-fix 轮）**：`DeliverPendingRentals` 终轮返回
    ...
    2026-09-08 补充：风控/限频/认证失效/UK失效/版本门禁/验证码哨兵在首轮即
    终止当前轮询和剩余待发货批次，不等重试耗尽；普通读取失败仍有限重试。
...
17. **共享客户端并发（2026-09-08 审查修复）**：健康检查会刷新 UserID/Nickname，
    这两个字段的读取与写入统一使用客户端元数据锁；行情/发货读取不得绕过
    `UserID()`。Registry 的 map 锁不能替代客户端内部同步。
18. **库存完整快照（2026-09-08）**：按每页 1000 条读到短页；缺失/null
    `ItemsInfos`、任一页错误、超过 500 页均返回错误，不提交部分数据。
    成功且显式空数组才是可用于清理旧库存状态的空快照。
19. **UU 初始化恢复（2026-09-08）**：Registry 独立保存“已配置但暂不可用”状态；
    首次验证失败不安装 adapter，独立 `channel_recovery` 任务每分钟检查，
    按 1 分钟起指数退避、上限 30 分钟重新验证。相同 token 的瞬态验证故障
    保留已有已验证客户端；确定认证失效或凭证已变则停止使用旧客户端。
    凭证更新、Refresh 和恢复安装串行化，避免旧验证结果覆盖新 token。
    更新 ECO 凭证只重建 ECO，不再触发 UU 网络验证。
...
- 2026-09-08 补全值校验：`Code/code=null` 同样属于协议错误，不能利用 Go
  `json.Unmarshal(null, &int)` 的零值语义变成成功。
