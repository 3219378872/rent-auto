...
- 发布本轮代码时，启动迁移增加货架观测水位、退架观测时间及订单反馈归属字段。
  本地迁移升降级测试只针对隔离测试库，不能用于生产库。
- 旧订单的首次观测字段由既有 updated_at 初始化；已应用的订单标记及现有因子保持不变。
  升级不能自动识别此前误折算的金额/因子，不应批量清空 factor_applied 或重置因子。
- `factor orders awaiting unique listing identity` 告警表示存在缺少资产/时间/挂单或归属歧义
  的终态订单；正常订单仍继续处理。核查原始订单及历史 goods_ref，优先通过后续平台同步
  补全事实。没有证据时保持未绑定，不将订单强行归给最新挂单。
- 回退 0011 会删除新增归属与观测信息；应先停调度、备份，并将应用与 schema 一并回退。
  新应用依赖这些列，不能在新应用运行时单独回退数据库。
